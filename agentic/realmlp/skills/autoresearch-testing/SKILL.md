...
The canonical runner is `bin/realmlp/test.py`, driven only on
...
`bin/realmlp/test.py` builds every run from a fixed `BASE_MODEL`
...
```bash
uv run bin/realmlp/test.py --<stage>-idx <idx>
```

Examples:

```bash
uv run bin/realmlp/test.py --model-idx 1
uv run bin/realmlp/test.py --optimizer-idx 1
uv run bin/realmlp/test.py --numerical-preprocess-idx 1
uv run bin/realmlp/test.py --loss-idx 1
```
...
```bash
uv run bin/realmlp/test.py --<stage>-idx <idx> --long-epochs 20
```
...
```bash
uv run bin/realmlp/test.py --exhaustive
```
