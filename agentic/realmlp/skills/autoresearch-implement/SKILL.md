...
description: Implement and register one module variant for a pipeline stage under bin/realmlp/modules, mirroring the v0 interface and keeping v0 and the base model schema intact. Use once per idea, before testing it.
---
...
1. Add a new file under the matching
   `bin/realmlp/modules/<stage>/` package. For a
   task-dispatched stage (`target_preprocess`, `inference`, `loss`), add it to
   the correct `_clf` or `_reg` package.
2. Mirror the `v0` interface for that stage — the v0 file (`realmlp.py` inside
   the stage package) states, in its docstring/signature, the exact builder
   signature and returned-object contract — and export a uniquely named
   builder, function, or class for the new variant.
...
- Do not change the wrapper `bin/realmlp/assemble.py`, the
  registry (`modules/__init__.py`), the shared helpers
  (`modules/preprocess.py`, `modules/_schedules.py`), or the
  base model config schema just because a new module does not use some keys.
