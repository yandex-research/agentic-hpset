...
- Stay within the model family: the base is an deep ensemble of MLPs. The final implementation of every module must keep it a single FC net (no RNNs or Transformers).
...
- Evidence bar: RealMLP-TD is meta-tuned. Its defaults were jointly optimized across a large benchmark suite, so the prior on any plausible-sounding change is that it may hurt. No idea is implemented without named, established support and a specific gap that the meta-tuned defaults cannot cover (see `autoresearch-hypothesis`).

## Research loop

Drive the loop by invoking these skills in order:

1. **Inspect** — invoke `autoresearch-inspect` to read the pipeline and list the live registries, internalizing the stages, the existing modules, and *why* the meta-tuned v0 choices look the way they do.
...
4. **Exhaustive check** — once all kept modules are implemented, invoke `autoresearch-testing` for the exhaustive interaction sweep (`uv run bin/realmlp/test.py --exhaustive`) and review the combined efficiency output.
