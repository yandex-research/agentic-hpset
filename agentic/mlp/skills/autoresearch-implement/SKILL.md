...
description: Implement and register one module variant for a pipeline stage under bin/mlp/modules, mirroring the v0 interface and keeping v0 and the base model schema intact. Use once per idea, before testing it.
---
...
1. Add a new file under the matching
   `bin/mlp/modules/<stage>/` package. For a
...
- Do not change the wrapper `bin/mlp/pipeline.py`, the registries in
  `bin/mlp/hparams.py`, any shared helpers a stage imports, or the base model
  config schema just because a new module does not use some keys. Variants plug
  into a stage registry only.
...
- The module must tolerate the full `cfg` (model config) dict supplied by the
  ...
  read from `cfg` with an in-code default. Keep those defaults reasonable.
