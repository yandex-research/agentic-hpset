...
The canonical runner is `bin/mlp/test.py`, driven only on
...
`bin/mlp/test.py` builds every run from a fixed `BASE_MODEL`
...
```bash
uv run bin/mlp/test.py --<stage>-idx <idx>
```

Examples:

```bash
uv run bin/mlp/test.py --model-idx 1
uv run bin/mlp/test.py --optimizer-idx 1
uv run bin/mlp/test.py --numerical-preprocess-idx 1
uv run bin/mlp/test.py --loss-idx 1
```
...
```bash
uv run bin/mlp/test.py --<stage>-idx <idx> --long-epochs 20
```
...
```bash
uv run bin/mlp/test.py --exhaustive
```
