...
def _init_scaling_(
    ...
    if sum(chunks) != tensor.shape[-1]:
        raise ValueError(
            "Scaling chunks must sum to the last tensor dimension, "
            f"got {sum(chunks)} and {tensor.shape[-1]}."
        )
...
class ElementwiseAffine(nn.Module):
    def __init__(
        ...
        *,
        bias: bool,
        scaling_init: str,
        scaling_init_chunks: list[int] | None = None,
    ) -> None:
        super().__init__()
...
class EnsembleView(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if x.ndim == 3 and x.shape[1] == self.k:
            return x
        raise ValueError(
            f"Expected shape (batch, d) or (batch, {self.k}, d), got {tuple(x.shape)}."
        )
...
class LinearEnsemble(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if x.ndim != 3 or x.shape[1] != self.weight.shape[0]:
            raise ValueError(
                f"Expected input shape (batch, {self.weight.shape[0]}, d), got {tuple(x.shape)}."
            )
        x = x.transpose(0, 1)
        x = x @ self.weight
        x = x.transpose(0, 1)
...
class TabMMiniBackbone(nn.Module):
    def __init__(
        self,
        *,
        d_in: int,
        ...
        if self.n_num_features:
            if self.num_embedding is None:
                ...
                if not hasattr(self.num_embedding, "get_output_shape"):
                    raise ValueError("Numerical embedding must define get_output_shape() for TabM.")
