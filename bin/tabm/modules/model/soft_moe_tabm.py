...
class SoftMoEBlock(nn.Module):
    """Soft mixture of E experts. Each expert is a Linear(d_in, d_out)+ReLU.

    A small gating Linear maps x -> E softmax weights, then the block returns the
    weighted sum of expert outputs followed by dropout. Soft (not top-k) gating
    keeps gradients dense and avoids the load-balancing instabilities of sparse
    MoE for small batch sizes.
    """

    def __init__(self, d_in: int, d_out: int, n_experts: int, dropout: float) -> None:
        super().__init__()
        self.n_experts = int(n_experts)
        # Single batched linear of shape (n_experts, d_in, d_out) for efficiency.
        self.expert_weight = nn.Parameter(
            torch.empty(self.n_experts, d_in, d_out)
        )
        self.expert_bias = nn.Parameter(torch.zeros(self.n_experts, d_out))
        bound = d_in**-0.5
        nn.init.uniform_(self.expert_weight, -bound, bound)
        self.gate = nn.Linear(d_in, self.n_experts)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x: Tensor) -> Tensor:
        # x: (..., d_in).  Compute expert outputs in a batched matmul.
        # einsum: (..., d_in) and (E, d_in, d_out) -> (..., E, d_out)
        expert_out = torch.einsum("...i,eij->...ej", x, self.expert_weight) + self.expert_bias
        expert_out = F.relu(expert_out)
        gate_logits = self.gate(x)  # (..., E)
        weights = F.softmax(gate_logits, dim=-1).unsqueeze(-1)  # (..., E, 1)
        mixed = (expert_out * weights).sum(dim=-2)  # (..., d_out)
        return self.dropout(mixed)


class TabMSoftMoEBackbone(nn.Module):
    def __init__(
        self,
        *,
        d_in: int,
        ...
        for _ in range(int(n_blocks)):
            blocks.append(
                SoftMoEBlock(current_dim, int(d_block), int(n_experts), float(dropout))
            )
...
class TabMSoftMoEModel(nn.Module):
    def __init__(
        self,
        *,
        ...
        self.backbone = TabMSoftMoEBackbone(
            d_in=sum(d_features),
            k=self.k,
...
def build_tabm_v4(
    ...
    """TabM with soft mixture-of-experts (E=4) per block.

    Hypothesis: tabular regression often involves piecewise behavior across
    different regions of feature space (different customer segments, regimes,
    etc.). A small MoE inside each feed-forward block lets the network dedicate experts
    to different regions while soft gating keeps gradients dense and avoids the
    load-balancing instabilities that plague top-k MoE on small batches.
    """
    model = TabMSoftMoEModel(
