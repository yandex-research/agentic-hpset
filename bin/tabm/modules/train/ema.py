...
@torch.no_grad()
def _ema_update(
    ema_state: dict[str, Tensor], model_state: dict[str, Tensor], decay: float
) -> None:
    for key, current in model_state.items():
        if key not in ema_state:
            ema_state[key] = current.detach().clone()
        else:
            running = ema_state[key]
            if running.dtype.is_floating_point:
                running.mul_(decay).add_(current.detach(), alpha=1.0 - decay)
            else:
...
def train_v4(
    ...
    ema_decay = float(trial_params.get("ema_decay", 0.999))
