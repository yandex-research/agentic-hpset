# ruff: noqa
"""Standalone implementation for ``build_densenet_mlp``."""
...
class DenseNetMLPModel(nn.Module):
    def __init__(
        self,
        ...
        for _ in range(n_blocks):
            self.blocks.append(
                nn.Sequential(
                    nn.Linear(current, d_block), nn.ReLU(), nn.Dropout(dropout)
                )
            )
            current = current + d_block
...
def build_densenet_mlp(
    _dataset_meta: Any,
    trial_params: dict[str, Any],
    embedding_bundle: dict[str, Any],
    device: torch.device,
) -> nn.Module:
    model = DenseNetMLPModel(
