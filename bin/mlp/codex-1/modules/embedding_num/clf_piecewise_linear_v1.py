...
class ClippedPiecewiseLinearEncoding(nn.Module):
    def __init__(self, bins: list[Tensor]) -> None:
        ...
        dtype = bins[0].dtype
        self.register_buffer('left', torch.stack([edges[0] for edges in bins]))
        self.register_buffer('right', torch.stack([edges[-1] for edges in bins]))
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = torch.minimum(torch.maximum(x, self.left), self.right)
...
class PLREmbeddings(nn.Module):
    def __init__(self, bins: list[Tensor], d_embedding: int) -> None:
        ...
        self.encoding = ClippedPiecewiseLinearEncoding(bins)
...
def compute_bins(x: Tensor | None, n_bins: int) -> list[Tensor]:
    ...
    for column in torch.unbind(x, dim=1):
        low = torch.quantile(
            column, torch.tensor(0.005, device=x.device, dtype=x.dtype)
        )
        high = torch.quantile(
            column, torch.tensor(0.995, device=x.device, dtype=x.dtype)
        )
        if high <= low:
            high = low + torch.tensor(1e-06, device=x.device, dtype=x.dtype)
        edges = torch.quantile(column.clamp(low, high), q).unique()
        if len(edges) < 2:
            edges = torch.stack([low, high])
        bins.append(edges)
    return bins
