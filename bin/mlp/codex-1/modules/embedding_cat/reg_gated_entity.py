...
class GatedEntityEmbeddings(nn.Module):
    def __init__(self, cardinalities: list[int], d_embedding: int) -> None:
        ...
        self.gates = nn.ParameterList(
            [nn.Parameter(torch.full((dim,), 2.0)) for dim in dims]
        )
        self.output_dim = sum(dims)

    def forward(self, x: Tensor) -> Tensor:
        pieces = []
        for i, (cardinality, embedding, gate) in enumerate(
            zip(self.cardinalities, self.embeddings, self.gates, strict=True)
        ):
            values = x[:, i].clamp(0, cardinality)
            pieces.append(embedding(values) * torch.sigmoid(gate))
...
def build_cat_embedding_v4(
    ...
    embedding = GatedEntityEmbeddings(
        cat_cardinalities, int(trial_params['d_embedding'])
    )
