...
def categorical_preprocess_v1(
    ...
    min_count = max(2, int(round(0.0025 * n_rows)))
    column_maps: list[np.ndarray] = []
    for column in range(encoded['train'].shape[1]):
        train_col = encoded['train'][:, column]
        train_col = train_col[train_col != unknown_value]
        max_code = int(train_col.max()) if train_col.size else -1
        counts = (
            np.bincount(train_col, minlength=max_code + 1)
            if max_code >= 0
            else np.array([])
        )
