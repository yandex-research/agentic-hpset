"""Search space, per-dataset defaults, and sampler for the modular MLP.
...
The result JSONs under ``exp/tuned/mlp`` and ``exp/tuned/agentic-mlp`` record the
sampled ``implementation_indices`` and ``trial_params``; feed those back through
``bin.mlp.pipeline`` to reconstruct any evaluated model. Index 0 of every stage is
the base reference implementation, so the non-agentic ``mlp`` runs are this same
...
from bin.mlp.modules.embedding_cat import CAT_EMBEDDING_MAP
from bin.mlp.modules.embedding_num import NUM_EMBEDDING_MAP
from bin.mlp.modules.inference_clf import INFERENCE_MAP as INFERENCE_CLF_MAP
from bin.mlp.modules.inference_reg import INFERENCE_MAP as INFERENCE_REG_MAP
from bin.mlp.modules.loss_clf import LOSS_CLF_MAP
from bin.mlp.modules.loss_reg import LOSS_REG_MAP
from bin.mlp.modules.model import MODEL_MAP
from bin.mlp.modules.optimizer import OPTIMIZER_MAP
from bin.mlp.modules.preprocess_categorical import CAT_PREPROCESS_MAP
from bin.mlp.modules.preprocess_numerical import NUM_PREPROCESS_MAP
from bin.mlp.modules.preprocess_target_clf import TARGET_PREPROCESS_CLF_MAP
from bin.mlp.modules.preprocess_target_reg import TARGET_PREPROCESS_REG_MAP
from bin.mlp.modules.train import TRAIN_MAP
...
# The per-stage index fields sampled per trial and recorded in each report's
...
# Stages whose registry is task-dispatched into ``_reg`` / ``_clf`` variants.
...
def get_default_hps(dataset_name: str) -> DefaultHPs:
    if dataset_name not in DATASET_TO_BATCH_SIZE:
        raise ValueError(
            ...
            "DATASET_TO_BATCH_SIZE in bin/mlp/hparams.py."
        )
    return DefaultHPs(
...
def sample_hps(
    ...
    *,
    ...
    The continuous/integer ranges here plus the per-stage categorical index
    choices are the full MLP search space. ``implementation_indices`` selects
    which candidate module each pipeline stage uses.
    """
