...
class SEBlock(nn.Module):
    def __init__(
        self, in_dim: int, out_dim: int, dropout: float, reduction: int = 4
    ) -> None:
        super().__init__()
        self.linear = nn.Linear(in_dim, out_dim)
        self.act = nn.ReLU()
        self.dropout = nn.Dropout(dropout)
        hidden = max(out_dim // reduction, 4)
        self.squeeze = nn.Linear(out_dim, hidden)
        self.excite = nn.Linear(hidden, out_dim)

    def forward(self, x: Tensor) -> Tensor:
        x = self.act(self.linear(x))
        gate = torch.sigmoid(self.excite(torch.relu(self.squeeze(x))))
        return self.dropout(x * gate)


class MLPSEModel(nn.Module):
    def __init__(
        self,
        ...
        for _ in range(n_blocks):
            blocks.append(SEBlock(current, d_block, dropout))
            current = d_block
