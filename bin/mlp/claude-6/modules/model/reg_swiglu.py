...
class SwiGLUBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        gate_input = torch.nan_to_num(self.gate(x), nan=0.0, posinf=20.0, neginf=-20.0)
        value_input = torch.nan_to_num(
            self.value(x), nan=0.0, posinf=20.0, neginf=-20.0
        )
        gate = F.silu(gate_input.clamp(-10.0, 10.0)).clamp(-5.0, 5.0)
        return self.dropout(gate * torch.tanh(value_input))
...
class SwiGLUModel(nn.Module):
    def __init__(
        ...
        for _ in range(n_blocks):
            blocks.append(SwiGLUBlock(current, d_block, dropout))
            current = d_block
