...
class ResNetBlock(nn.Module):
    def __init__(
        self, d_block: int, d_hidden: int, dropout: float, residual_scale: float
    ) -> None:
        super().__init__()
        self.norm = nn.LayerNorm(d_block)
        self.linear1 = nn.Linear(d_block, d_hidden)
        self.linear2 = nn.Linear(d_hidden, d_block)
        self.dropout = nn.Dropout(dropout)
        self.activation = nn.ReLU()
        self.residual_scale = residual_scale

    def forward(self, x: Tensor) -> Tensor:
        update = self.dropout(self.linear2(self.activation(self.linear1(self.norm(x)))))
...
class ResNetModel(nn.Module):
    ...
    def forward(self, x_num: Tensor | None, x_cat: Tensor | None) -> Tensor:
        ...
        for block in self.blocks:
            x = block(x)
        return self.output(self.norm(x)).squeeze(-1)
