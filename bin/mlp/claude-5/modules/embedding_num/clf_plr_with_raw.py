...
class PLRWithRawEmbeddings(nn.Module):
    """PLR + projected raw value channel concatenated to PLR's per-feature embedding."""
    ...
    def __init__(self, bins: list[Tensor], d_embedding: int) -> None:
        ...
        self.raw_proj = LinearEmbeddings(n_features, d_embedding)
        self.gate = nn.Parameter(torch.full((n_features, d_embedding), 0.0))

    def forward(self, x: Tensor) -> Tensor:
        x_linear = self.linear0(x)
        x_ple = self.encoding(x).transpose(0, 1)
        x_ple = (x_ple @ self.linear).transpose(0, 1)
        x_raw = self.raw_proj(x)
        gate = torch.sigmoid(self.gate)
        return x_linear + x_ple + gate * x_raw
...
def build_num_embedding_v4(
    ...
    embedding = PLRWithRawEmbeddings(bins, int(trial_params['d_embedding']))
