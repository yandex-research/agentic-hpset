...
class TopKReLU(nn.Module):
    """Sparse activation: keep top-k% positive activations per sample, zero the rest.

    Forces feature-level sparsity in MLP intermediate layers, which can act like
    induced interaction sparsity and improve generalization on tabular data.
    """
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.keep_ratio >= 1.0:
            return x
        d = x.shape[-1]
        k = max(1, int(round(d * self.keep_ratio)))
        if k >= d:
            return x
        values, indices = x.topk(k, dim=-1)
        masked = torch.zeros_like(x).scatter(-1, indices, values)
        return masked * math.sqrt(d / k)


class TopKMLPModel(nn.Module):
    def __init__(
        self,
        ...
        keep_ratio = float(params.get('activation_keep', 0.5))
        blocks: list[nn.Module] = []
        current = input_dim
        for _ in range(n_blocks):
            blocks.extend(
                [nn.Linear(current, d_block), TopKReLU(keep_ratio), nn.Dropout(dropout)]
            )
            current = d_block
