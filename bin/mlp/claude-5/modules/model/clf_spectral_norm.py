...
class SpectralNormMLPModel(nn.Module):
    """MLP backbone with spectral-norm-constrained linear layers.

    Spectral norm constrains the largest singular value to 1, regularizing
    sensitivity and improving generalization by enforcing Lipschitz-like
    ...
    def __init__(
        ...
        for _ in range(n_blocks):
            blocks.extend(
                [
                    _spectral_norm(nn.Linear(current, d_block)),
                    nn.ReLU(),
                    nn.Dropout(dropout),
                ]
            )
