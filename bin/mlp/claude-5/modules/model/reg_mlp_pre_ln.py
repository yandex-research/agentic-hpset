...
class PreLNMLPModel(nn.Module):
    """MLP with LayerNorm applied before each linear block. This is the
    pre-norm convention from modern transformer training: every block sees a
    standardized input, which keeps activations on a consistent scale even
    when stacking many blocks. For tabular MLPs that train with relatively
    short schedules (and where input variance can vary per dataset) this can
    make optimization noticeably steadier."""
