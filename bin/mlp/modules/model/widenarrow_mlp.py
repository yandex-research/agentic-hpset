...
class WideNarrowMLP(nn.Module):
    """Wide-narrow alternating MLP backbone.

    Each block performs a Transformer-FFN-style wide→narrow mapping:
    Linear(d, 2*d) → ReLU → Dropout → Linear(2*d, d) → ReLU → Dropout.
    Roughly the same total parameter budget as a uniform-width MLP but with
    a richer per-block transformation.
    """
    ...
    def __init__(
        ...
        *,
        ...
        for _ in range(n_blocks):
            blocks.extend(
                [
                    nn.Linear(current, d_wide),
                    nn.ReLU(),
                    nn.Dropout(dropout),
                    nn.Linear(d_wide, d_block),
                    nn.ReLU(),
                    nn.Dropout(dropout),
                ]
            )
