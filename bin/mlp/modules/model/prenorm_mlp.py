...
class PreNormMLPModel(nn.Module):
    """Pre-LayerNorm MLP.

    LayerNorm is applied before each backbone linear, plus a final norm before
    the output head. Pre-norm stabilizes training of deeper MLPs and reduces
    ...
    def forward(self, x_num: Tensor | None, x_cat: Tensor | None) -> Tensor:
        ...
        return self.output(self.final_norm(self.backbone(x))).squeeze(-1)
