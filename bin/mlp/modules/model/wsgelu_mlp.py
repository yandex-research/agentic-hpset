...
class WSGELUMLP(nn.Module):
    """MLP with Weight-Standardization + GELU and zero-init output head.

    Replaces ReLU with GELU, standardizes the input dim of every backbone
    weight at each forward pass, and zero-initializes the output linear so
    early training starts from a uniform-logit state.
    """
    ...
    def __init__(
        ...
        *,
        ...
        for _ in range(n_blocks):
            blocks.extend([WSLinear(current, d_block), nn.GELU(), nn.Dropout(dropout)])
            current = d_block
