...
def _ensemble_predictions(
    ...
    per_state: list[dict[str, np.ndarray]] = []
    for _neg_score, _epoch, state in top_states:
        model.load_state_dict(state)
        model.eval()
        with torch.no_grad():
            preds = inference_fn(model, dataset_tensors, dataset_meta, trial_params, device, parts)
        per_state.append(preds)
    averaged: dict[str, np.ndarray] = {}
    for part in parts:
        stacked = np.stack([p[part] for p in per_state], axis=0)
        averaged[part] = stacked.mean(axis=0).astype(np.float32)
    return averaged


def train_ensemble_v0(
    model: nn.Module,
    dataset_tensors: TorchDataset,
    dataset_meta: Dataset,
    trial_params: dict[str, Any],
    ...
    for epoch in range(1, int(trial_params["max_epochs"]) + 1):
        ...
        # One deepcopy per epoch, shared by best_state and heap entry when both apply.
        # Safe because state dicts are only loaded, never mutated.
        state_copy = (
    ...
    averaged = _ensemble_predictions(
        model, top_states, inference_fn, dataset_tensors, dataset_meta,
        trial_params, device, ("train", "val", "test"),
    )
    metrics = compute_metrics_for_parts(dataset_meta, averaged, ("train", "val", "test"))
