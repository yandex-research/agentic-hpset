"""Stochastic Weight Perturbation (SWP): cheap flat-minima proxy for SAM.

On each step, every parameter is perturbed in-place with multiplicative
Gaussian noise scaled by the parameter's running magnitude. A single
forward and backward pass is run through the perturbed model; the original
weights are restored before the optimizer applies its update. The gradient
is therefore evaluated at a noisy neighborhood of the current weights,
biasing optimization toward flat minima at the cost of one extra parameter
copy per step (no extra forward, unlike SAM).
"""
...
def train_swp_v0(
    ...
    sigma = float(trial_params.get("swp_sigma", 0.01))
        ...
        for batch_idx in permutation.split(batch_size):
            ...
            with torch.no_grad():
                for p in model.parameters():
                    ...
                    scale = sigma * (p.data.detach().abs().mean().clamp_min(1e-8))
