...
def train_swa_v0(
    ...
    swa_start_epoch = max(1, int(max_epochs * 0.75))
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if epoch >= swa_start_epoch:
            ...
            if swa_state is None:
                swa_state = {
                    k: v.float().clone() if v.is_floating_point() else v.clone()
                ...
                swa_count += 1
                for k in swa_state:
                    if current_sd[k].is_floating_point():
                        swa_state[k] += (current_sd[k].float() - swa_state[k]) / swa_count
                    else:
                        swa_state[k] = current_sd[k].clone()
        model.eval()
        with torch.no_grad():
    ...
    if swa_state is not None:
        ...
        model.eval()
        with torch.no_grad():
            swa_predictions = inference_fn(
                model, dataset_tensors, dataset_meta, trial_params, device, ("val",)
            )
        swa_metrics = compute_metrics_for_parts(dataset_meta, swa_predictions, ("val",))
        if float(swa_metrics["val"]["score"]) < best_score:
            best_state = {k: v.clone() for k, v in swa_state.items()}
            best_epoch = -1
