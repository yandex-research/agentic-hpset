...
def _add_gradient_noise(model: nn.Module, noise_scale: float) -> None:
    for p in model.parameters():
        if p.grad is not None:
            p.grad.add_(torch.randn_like(p.grad) * noise_scale)


def train_gradnoise_v0(
    model: nn.Module,
    ...
    noise_eta = float(optimizer.param_groups[0]["lr"]) * 0.1
    ...
    for epoch in range(1, int(trial_params["max_epochs"]) + 1):
        ...
        noise_scale = noise_eta / (1.0 + epoch) ** 0.55
        for batch_idx in permutation.split(batch_size):
            ...
            loss.backward()
            _add_gradient_noise(model, noise_scale)
