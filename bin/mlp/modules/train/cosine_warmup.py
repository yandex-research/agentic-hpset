"""Trainer with cosine LR schedule and linear warmup.

Linear warmup over the first 10% of training steps, cosine decay over the
remaining 90%. The warmup reduces divergence risk on freshly initialized
networks; the cosine tail anneals into a sharper minimum. All other
training mechanics match the base trainer.
"""
...
def train_cosine_warmup_v0(
    ...
    total_steps = max(1, steps_per_epoch * int(trial_params["max_epochs"]))
    warmup_steps = max(1, int(0.1 * total_steps))
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer, _make_lr_lambda(total_steps, warmup_steps)
    )
