...
def categorical_preprocess_v4(
    ...
    min_count = max(2, int(0.005 * n_rows))
    ...
    for column in range(encoded['train'].shape[1]):
        ...
        mapping = np.full(n_train_categories + 1, int(frequent.sum()), dtype=np.int64)
        mapping[np.where(frequent)[0]] = np.arange(int(frequent.sum()), dtype=np.int64)
        mappings.append(mapping)
        for part, values in encoded.items():
            col = values[:, column]
            safe_col = np.where(col >= n_train_categories, n_train_categories, col)
            rare_parts[part][:, column] = mapping[safe_col]
    transformed = {
        part: np.column_stack([encoded[part], rare_parts[part]]).astype(np.int64)
        for part in encoded
    }
    return (
        transformed,
        {'mappings': mappings, 'min_count': min_count, 'unknown_value': unknown_value},
    )
