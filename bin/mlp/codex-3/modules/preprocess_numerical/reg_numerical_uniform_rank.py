...
def numerical_preprocess_v2(
    ...
    transformer = sklearn.preprocessing.QuantileTransformer(
        n_quantiles=max(min(x_train.shape[0] // 15, 1000), 10),
        output_distribution='uniform',
        subsample=1000000000,
        random_state=seed,
    )
    noise = np.random.RandomState(seed + 31).normal(0.0, 1e-05, x_train.shape)
    transformer.fit(x_train + noise.astype(x_train.dtype))
    transformed = {
        part: (2.0 * np.nan_to_num(transformer.transform(values)) - 1.0).astype(
            np.float32
        )
        for part, values in filled.items()
    }
    transformed = {
        part: np.clip(values, -1.0, 1.0).astype(np.float32)
