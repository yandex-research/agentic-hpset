...
class DenseSkipMLP(nn.Module):
    def __init__(
        ...
        for block_idx in range(n_blocks):
            block_input = current if block_idx == 0 else current + input_dim
            self.blocks.append(
                nn.Sequential(
                    nn.Linear(block_input, d_block), nn.ReLU(), nn.Dropout(dropout)
                )
            )
            current = d_block
    ...
    def forward(self, x_num: Tensor | None, x_cat: Tensor | None) -> Tensor:
        x0 = self._features(x_num, x_cat)
        x = x0
        for block_idx, block in enumerate(self.blocks):
            x = block(x if block_idx == 0 else torch.cat([x, x0], dim=1))
