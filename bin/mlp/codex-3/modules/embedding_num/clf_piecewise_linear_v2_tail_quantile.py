...
def _safe_unique_edges(edges: Tensor) -> Tensor:
    edges = edges.unique()
    if edges.numel() < 2:
        edge = edges[0] if edges.numel() else torch.tensor(0.0, device=edges.device)
        return torch.stack([edge, edge + torch.ones_like(edge)])
    return edges
...
def compute_bins(x: Tensor | None, n_bins: int) -> list[Tensor]:
    if x is None:
        return []
    base = torch.linspace(0.0, 1.0, n_bins + 1, device=x.device, dtype=x.dtype)
    quantiles = 0.5 - 0.5 * torch.cos(torch.pi * base)
    return [
        _safe_unique_edges(edges) for edges in torch.quantile(x, quantiles, dim=0).T
    ]
