...
"""Standalone implementation for ``build_num_embedding_v0``."""
...
class PiecewiseLinearEncoding(nn.Module):
    def __init__(self, bins: list[Tensor]) -> None:
        ...
        self.register_buffer('weight', torch.zeros(len(bins), max_n_bins))
        self.register_buffer('bias', torch.zeros(len(bins), max_n_bins))
        self.register_buffer('single_bin_mask', torch.tensor(n_bins) == 1)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return encoded if self.mask is None else encoded * self.mask
...
class PLREmbeddings(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return x_linear + (x_ple @ self.linear).transpose(0, 1)
...
def compute_bins(x: Tensor | None, n_bins: int) -> list[Tensor]:
    ...
    return [edges.unique() for edges in torch.quantile(x, quantiles, dim=0).T]
