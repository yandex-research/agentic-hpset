...
class GatedPLREmbeddings(nn.Module):
    def __init__(self, bins: list[Tensor], d_embedding: int) -> None:
        ...
        self.gate = nn.Parameter(torch.zeros(len(bins), d_embedding))

    def forward(self, x: Tensor) -> Tensor:
        x_linear = self.linear0(x)
        x_ple = self.encoding(x).transpose(0, 1)
        x_ple = (x_ple @ self.linear).transpose(0, 1)
        return x_linear + torch.sigmoid(self.gate) * x_ple
...
def build_num_embedding_v2(
    ...
    embedding = GatedPLREmbeddings(bins, d_embedding)
    return (embedding, dataset_meta.n_num_features * d_embedding, {'bins': bins})
