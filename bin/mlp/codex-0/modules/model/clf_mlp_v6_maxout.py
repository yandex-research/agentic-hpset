...
class MaxoutBlock(nn.Module):
    def __init__(
        self, input_dim: int, output_dim: int, dropout: float, pieces: int = 2
    ) -> None:
        super().__init__()
        self.output_dim = output_dim
        self.pieces = pieces
        self.linear = nn.Linear(input_dim, output_dim * pieces)
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = (
            self.linear(x)
            .view(x.shape[0], self.output_dim, self.pieces)
            .max(dim=-1)
            .values
        )
...
class MaxoutMLPModel(nn.Module):
    def __init__(
        self,
        ...
        for _ in range(n_blocks):
            layers.append(MaxoutBlock(current, d_block, dropout))
            current = d_block
