# ruff: noqa
"""Standalone implementation for ``build_mlp_v10``."""
...
class InputNormLeakyMLPModel(nn.Module):
    def __init__(
        self,
        ...
        for _ in range(n_blocks):
            blocks.extend(
                [nn.Linear(current, d_block), nn.LeakyReLU(0.05), nn.Dropout(dropout)]
            )
            current = d_block
    ...
    def forward(self, x_num: Tensor | None, x_cat: Tensor | None) -> Tensor:
        ...
        output = self.output(self.backbone(self.input_norm(x)))
