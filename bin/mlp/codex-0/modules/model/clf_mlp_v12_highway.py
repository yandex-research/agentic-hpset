# ruff: noqa
"""Standalone implementation for ``build_mlp_v12``."""
...
class HighwayBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return self.norm(gate * self.transform(x) + (1.0 - gate) * self.carry(x))


class HighwayMLPModel(nn.Module):
    def __init__(
        self,
        ...
        for _ in range(n_blocks):
            layers.append(HighwayBlock(current, d_block, dropout))
            current = d_block
