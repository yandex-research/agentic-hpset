# ruff: noqa
"""Standalone implementation for ``build_model_v2``."""
...
class CrossMLP(nn.Module):
    def __init__(
        ...
        rank = max(16, min(256, int(params['d_block']) // 4))
        self.cross_layers = nn.ModuleList(
            [CrossLayer(input_dim, rank) for _ in range(2)]
        )
        self.backbone, current = _mlp_layers(input_dim, params)
        self.output = nn.Linear(current + input_dim, output_dim)

    def forward(self, x_num: Tensor | None, x_cat: Tensor | None) -> Tensor:
        x0 = _assemble_input(self.num_embedding, self.cat_embedding, x_num, x_cat)
        crossed = x0
        for layer in self.cross_layers:
            crossed = layer(x0, crossed)
        output = self.output(torch.cat([self.backbone(crossed), crossed], dim=1))
