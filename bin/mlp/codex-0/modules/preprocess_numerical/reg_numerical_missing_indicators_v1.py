# ruff: noqa
"""Standalone implementation for ``numerical_missing_indicators_v1``."""
...
def numerical_missing_indicators_v1(
    x_num: dict[str, np.ndarray] | None, config: dict[str, Any] | None = None
) -> tuple[dict[str, np.ndarray] | None, dict[str, object]]:
    if x_num is None:
        return (None, {'base': None, 'indicator_mask': None})
    base, artifacts = numerical_preprocess_v0(x_num, config)
    indicator_mask = np.isnan(x_num['train']).any(axis=0)
    if not indicator_mask.any():
        return (base, {'base': artifacts, 'indicator_mask': indicator_mask})
    indicators = {
        part: np.isnan(values[:, indicator_mask]).astype(np.float32)
        for part, values in x_num.items()
    }
    if base is None:
        return (indicators, {'base': artifacts, 'indicator_mask': indicator_mask})
    return (
        {
            part: np.column_stack([base[part], indicators[part]]).astype(np.float32)
            for part in base
        },
        {'base': artifacts, 'indicator_mask': indicator_mask},
    )
