# ruff: noqa
"""Standalone implementation for ``build_multihead_mlp``."""
...
class MultiHeadMLP(nn.Module):
    def __init__(
        ...
        self.heads = nn.ModuleList([nn.Linear(current, 1) for _ in range(n_heads)])

    def forward(self, x_num: Tensor | None, x_cat: Tensor | None) -> Tensor:
        x = _features(self.num_embedding, self.cat_embedding, x_num, x_cat)
        h = self.backbone(x)
        preds = torch.stack([head(h).squeeze(-1) for head in self.heads], dim=0)
...
def build_multihead_mlp(
    ...
    return _build(MultiHeadMLP, trial_params, embedding_bundle, device)
