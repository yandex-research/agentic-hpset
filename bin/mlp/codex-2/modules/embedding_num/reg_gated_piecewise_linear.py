# ruff: noqa
"""Standalone implementation for ``build_gated_piecewise_linear``."""
...
class GatedPiecewiseLinearEmbeddings(nn.Module):
    def __init__(self, bins: list[Tensor], d_embedding: int) -> None:
        super().__init__()
        self.base = PLREmbeddings(bins, d_embedding)
        self.gate_weight = nn.Parameter(torch.zeros(len(bins), d_embedding))
        self.gate_bias = nn.Parameter(torch.full((len(bins), d_embedding), 2.0))
        self.register_buffer('initial_gate', torch.sigmoid(torch.tensor(2.0)))

    def forward(self, x: Tensor) -> Tensor:
        gate = torch.sigmoid(
            torch.addcmul(self.gate_bias, self.gate_weight, x[..., None])
        )
        return self.base(x) * (gate / self.initial_gate)
...
def build_gated_piecewise_linear(
    ...
    if x_num_train is None or dataset_meta.n_num_features == 0:
        return (None, 0, {'variant': 'gated_piecewise_linear', 'bins': []})
    bins = compute_bins(x_num_train, int(trial_params['n_bins']))
    d_embedding = int(trial_params['d_embedding'])
    return (
        GatedPiecewiseLinearEmbeddings(bins, d_embedding),
        dataset_meta.n_num_features * d_embedding,
        {'variant': 'gated_piecewise_linear', 'bins': bins},
    )
