...
class PLREmbeddingsWithGlobalToken(nn.Module):
    """PLR with a learned per-row global token appended along the feature axis.

    The global token is computed by mixing per-feature embeddings via a small
    learnable summary: a learnable projection over features yields a single
    summary vector per row. This vector is concatenated as an extra
    "feature" in the embedding output, exposing cross-feature signal to the
    downstream MLP at zero cost beyond a small parameter count.
    """
    ...
    def __init__(self, bins: list[Tensor], d_embedding: int) -> None:
        ...
        self.summary_weight = nn.Parameter(torch.empty(n_features, 1))
        nn.init.uniform_(
            self.summary_weight,
            -1.0 / max(n_features, 1) ** 0.5,
            1.0 / max(n_features, 1) ** 0.5,
        )
        self.summary_bias = nn.Parameter(torch.zeros(d_embedding))

    def forward(self, x: Tensor) -> Tensor:
        x_linear = self.linear0(x)
        x_ple = self.encoding(x).transpose(0, 1)
        x_ple = (x_ple @ self.linear).transpose(0, 1)
        per_feature = x_linear + x_ple
        summary = (per_feature * self.summary_weight[None, :, :]).sum(
            dim=1
        ) + self.summary_bias
        out = torch.cat([per_feature, summary[:, None, :]], dim=1)
        return out
...
def build_num_embedding_v4(
    ...
    embedding = PLREmbeddingsWithGlobalToken(bins, int(trial_params['d_embedding']))
    output_dim = (dataset_meta.n_num_features + 1) * int(trial_params['d_embedding'])
