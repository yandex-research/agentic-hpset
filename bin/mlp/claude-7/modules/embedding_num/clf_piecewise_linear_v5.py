...
class PiecewiseQuadraticPLREmbeddings(nn.Module):
    """PLR with a piecewise *quadratic* term concatenated.

    The standard PLR encoding gives a per-bin local linear coordinate; here we
    additionally include the element-wise *square* of that coordinate before
    the per-feature linear projection. This lets the embedding represent
    bounded curvature inside each bin, not just slope, while keeping the
    PLR structure (bin assignment, monotone signal) intact.
    """
    ...
    def __init__(self, bins: list[Tensor], d_embedding: int) -> None:
        ...
        encoding_dim = self.encoding.weight.shape[1]
        self.linear = nn.Parameter(
            torch.zeros(n_features, 2 * encoding_dim, d_embedding)
        )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        combined = torch.cat([encoded, squared], dim=-1).transpose(0, 1)
        x_ple = (combined @ self.linear).transpose(0, 1)
...
def build_num_embedding_v5(
    ...
    embedding = PiecewiseQuadraticPLREmbeddings(bins, int(trial_params['d_embedding']))
