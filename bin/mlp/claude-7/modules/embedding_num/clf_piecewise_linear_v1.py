...
class PLREmbeddingsFeatureDrop(nn.Module):
    def __init__(
        self, bins: list[Tensor], d_embedding: int, feature_dropout: float
    ) -> None:
        super().__init__()
...
def build_num_embedding_v1(
    ...
    feature_dropout = 0.2
    embedding = PLREmbeddingsFeatureDrop(
        bins, int(trial_params['d_embedding']), feature_dropout=feature_dropout
    )
    output_dim = dataset_meta.n_num_features * int(trial_params['d_embedding'])
    return (embedding, output_dim, {'bins': bins, 'feature_dropout': feature_dropout})
