...
class MLPModelInputGate(nn.Module):
    """MLP with a learnable per-input-dimension sigmoid gate.

    A single learnable parameter g (one per input dim) gives sigmoid(g) as a
    soft feature-selection mask applied multiplicatively to the input. Acts
    like a continuous variant of feature selection that is fully optimizable
    via SGD and contributes negligible parameters.
    """
    ...
    def __init__(
        ...
        self.input_gate_logits = nn.Parameter(torch.zeros(input_dim))
    ...
    def forward(self, x_num: Tensor | None, x_cat: Tensor | None) -> Tensor:
        ...
        x = x * torch.sigmoid(self.input_gate_logits)[None, :]
