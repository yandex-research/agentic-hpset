# ruff: noqa
"""Standalone implementation for ``build_mlp_v3_gelu``."""
...
class GELUMLP(nn.Module):
    """MLP using GELU activations instead of ReLU.

    GELU is smoother than ReLU and matches what's used in modern transformer
    backbones; sometimes improves convergence on tabular tasks.
    """
    ...
    def __init__(
        ...
        for _ in range(n_blocks):
            blocks.extend([nn.Linear(current, d_block), nn.GELU(), nn.Dropout(dropout)])
            current = d_block
