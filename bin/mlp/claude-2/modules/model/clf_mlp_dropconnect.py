...
class DropConnectLinear(nn.Module):
    def __init__(self, d_in: int, d_out: int, drop_p: float) -> None:
        super().__init__()
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.training and self.drop_p > 0:
            mask = torch.empty_like(self.weight).bernoulli_(1.0 - self.drop_p)
            scale = 1.0 / max(1.0 - self.drop_p, 1e-06)
            w = self.weight * mask * scale
        else:
            w = self.weight
...
class DropConnectMLP(nn.Module):
    def __init__(
        ...
        drop_p = float(params['dropout'])
        blocks: list[nn.Module] = []
        current = input_dim
        for _ in range(n_blocks):
            blocks.extend([DropConnectLinear(current, d_block, drop_p), nn.ReLU()])
            current = d_block
