...
def numerical_preprocess_v2(
    ...
    """Winsorized standard scaling to blunt heavy-tailed regression covariates."""
    if x_num is None:
        return (None, {'variant': 'winsor_standard_missing', 'keep_mask': None})
    imputed, median = _median_impute(x_num)
    lo, hi = np.quantile(imputed['train'], [0.01, 0.99], axis=0)
    ...
    return _drop_constant(
        _concat(blocks),
        {
            'variant': 'winsor_standard_missing',
            'median': median,
            'lo': lo,
            'hi': hi,
            'mean': mean,
