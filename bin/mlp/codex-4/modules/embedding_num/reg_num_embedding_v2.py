...
class RawPlusPLREmbeddings(nn.Module):
    def __init__(self, bins: list[Tensor], d_embedding: int) -> None:
        super().__init__()
        self.plr = PLREmbeddings(bins, d_embedding)

    def forward(self, x: Tensor) -> Tensor:
        return torch.cat([x[..., None], self.plr(x)], dim=-1)
...
def build_num_embedding_v2(
    ...
    if x_num_train is None or dataset_meta.n_num_features == 0:
        return (None, 0, {'variant': 'raw_plus_plr', 'bins': []})
    bins = compute_bins(x_num_train, int(trial_params['n_bins']))
    d_embedding = int(trial_params['d_embedding'])
    return (
        RawPlusPLREmbeddings(bins, d_embedding),
        dataset_meta.n_num_features * (d_embedding + 1),
        {'variant': 'raw_plus_plr', 'bins': bins},
    )
